...
model_dir = "resnet50"
comparison_report_path = "resnet50/comparison_report.txt"
...
# ResNet model with task-specific heads and Dropout regularization
...
if __name__ == "__main__":
    ...
    train_df = encoder.fit_transform(train_df, label_columns)
    test_df = encoder.transform(test_df, label_columns)

    # Split train data into training and validation sets
    train_df, val_df = train_test_split(train_df, test_size=0.2, random_state=42, stratify=train_df[label_columns])
    ...
    val_dataset = SkinLesionDataset(val_df, train_img_dir, transform=val_test_transform)
    test_dataset = SkinLesionDataset(test_df, test_img_dir, transform=val_test_transform)
