...
model_dir = "googleNet"
comparison_report_path = "googleNet/comparison_report.txt"
...
# GoogleNet (Inception v3) model with task-specific heads and Dropout regularization
class GoogleNetModel(nn.Module):
    def __init__(self, num_classes_super_class=2, num_classes_malignancy=3, num_classes_main_class_1=7,
        ...
        self.main_features = self.base_model.fc.in_features  # 2048
        self.aux_features = self.base_model.AuxLogits.fc.in_features  # 768

        # Replace the final layer of main model
        self.base_model.fc = nn.Linear(self.main_features, 1024)

        # Replace auxiliary classifier
        self.base_model.AuxLogits.fc = nn.Linear(self.aux_features, 1024)
    ...
    def forward(self, x):
        # During training, the model returns (output, aux_output)
        if self.training:
        ...
        if self.training:
            # During training, add auxiliary outputs
            aux = self.dropout(aux)  # aux is already 1024-dimensional now
            aux_out_dict = {
                "super_class_aux": self.fc_super_class(aux),
                "malignancy_aux": self.fc_malignancy(aux),
                "main_class_1_aux": self.fc_main_class_1(aux),
                "main_class_2_aux": self.fc_main_class_2(aux),
                "sub_class_aux": self.fc_sub_class(aux)
            }
...
def train_specific_model(train_loader, val_loader, device):
    ...
    aux_criterion_dict = {
        "super_class_aux": nn.CrossEntropyLoss().to(device),
        "malignancy_aux": nn.CrossEntropyLoss().to(device),
        "main_class_1_aux": nn.CrossEntropyLoss().to(device),
        "main_class_2_aux": nn.CrossEntropyLoss().to(device),
        "sub_class_aux": nn.CrossEntropyLoss().to(device)
    }
    ...
    for epoch in range(1, 20):
        ...
        for images, labels in tqdm(train_loader, desc=f"Training Epoch {epoch}"):
            ...
            # Calculate main loss
            main_loss = sum(criterion_dict[task](outputs[task], labels[task])
                            for task in criterion_dict.keys())

            # Calculate auxiliary loss during training
            aux_loss = 0
            if model.training:
                aux_loss = sum(aux_criterion_dict[f"{task}_aux"](outputs[f"{task}_aux"], labels[task])
                               for task in criterion_dict.keys())

            # Combined loss (0.3 is the standard auxiliary loss weight for Inception)
