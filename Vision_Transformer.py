...
model_dir = "visionTransformer"
comparison_report_path = "visionTransformer/comparison_report.txt"
...
# Vision Transformer (ViT) model with task-specific heads and Dropout regularization
class VisionTransformerModel(nn.Module):
    def __init__(self, num_classes_super_class=2, num_classes_malignancy=3, num_classes_main_class_1=7,
        ...
        self.base_model.heads.head = nn.Linear(self.base_model.heads.head.in_features, 1024)
...
def train_specific_model(train_loader, val_loader, device):
    ...
    for epoch in range(1, 10):  # 20 epochs
        model.train()
        train_loss = 0
...
if __name__ == "__main__":
    ...
    train_transform = transforms.Compose([
        transforms.Resize((224, 224)),  # Update to 224
        transforms.RandomHorizontalFlip(),
        transforms.RandomVerticalFlip(),
        transforms.RandomRotation(degrees=222),
        transforms.ColorJitter(brightness=1.2802390610488672),
        transforms.RandomResizedCrop(size=224, scale=(0.8, 1.0)),  # Update to 224
        transforms.ToTensor(),
