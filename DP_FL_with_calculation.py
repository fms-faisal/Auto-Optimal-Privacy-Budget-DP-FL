...
NUM_CLIENTS = 3
NUM_ROUNDS = 20
CANDIDATE_EPSILONS = [0.5, 1, 2.0, 3.0, 3.5, 4.0, 5.0, 7.0, 10.0, 20.0]
w1, w2 = 0.5, 0.5
INITIAL_DATA_FRACTION = 0.1  # 0.1%
DATA_FRACTION_INCREMENT = 0.05  # 0.05 per round
...
def log_model_params(model, phase="Model Initialization"):
    total_params = sum(p.numel() for p in model.parameters())
    log_message(f"{phase} - Total parameters: {total_params}")

# Timer context manager that also logs resource usage
class Timer:
    ...
    def __exit__(self, *args):
        ...
        if torch.cuda.is_available():
            torch.cuda.synchronize()
            self.end_gpu = torch.cuda.memory_allocated()
        else:
            self.end_gpu = 0

        elapsed = self.end_time - self.start_time
        log_message(f"[{self.phase_name}] Time elapsed: {elapsed:.4f} seconds")
        log_message(f"[{self.phase_name}] CPU usage: start {self.start_cpu}% -> end {self.end_cpu}%")
        mem_diff = (self.end_mem - self.start_mem) / (1024**2)
        log_message(f"[{self.phase_name}] Memory usage change: {mem_diff:.2f} MB")
        if torch.cuda.is_available():
            gpu_diff = (self.end_gpu - self.start_gpu) / (1024**2)
            log_message(f"[{self.phase_name}] GPU memory usage change: {gpu_diff:.2f} MB")
...
class FlowerClient(fl.client.NumPyClient):
    def __init__(self, full_dataset, device):
        ...
        log_model_params(self.model, phase="FlowerClient Model Initialization")
    ...
    def _init_privacy_engine(self, epsilon, train_loader):
        with Timer("Privacy Engine Initialization"):
    ...
    def fit(self, parameters: NDArrays, config: Dict[str, str]) -> Tuple[NDArrays, int, Dict]:
        ...
        with Timer("Data Fraction Calculation and Subset Generation"):
            ...
            if len(subset) < 2:
                ...
                train_subset, _ = random_split(subset, [train_size, val_size])
            if len(train_subset) == 0:
                raise ValueError("train_subset is empty. Check your data_split logic.")
            train_loader = DataLoader(train_subset, batch_size=BATCH_SIZE, shuffle=True)

        train_loader = self._init_privacy_engine(new_epsilon, train_loader)

        with Timer("Total Client Training (Fit)"):
            for epoch in range(EPOCHS):
                with Timer(f"Epoch {epoch + 1} Training"):
                    ...
                    log_message(f"Client training - Epoch {epoch + 1}: Loss {epoch_loss:.4f}, Avg Accuracy {epoch_acc:.4f}, Epsilon {new_epsilon}")
...
class EpsilonAwareStrategy(fl.server.strategy.FedAvg):
    def __init__(self, server_proxy_dataset, server_val_dataset, test_loader, device, proxy_epochs=3, *args, **kwargs):
        ...
        self.current_epsilon = CANDIDATE_EPSILONS[5]
    ...
    def select_optimal_epsilon(self, parameters_ndarrays):
        with Timer("Optimal Epsilon Selection"):
            data_fraction = INITIAL_DATA_FRACTION + (self.current_round - 1) * DATA_FRACTION_INCREMENT
            data_fraction = min(data_fraction, 1.0)
            proxy_size = int(len(self.server_proxy_dataset) * data_fraction)
            proxy_size = max(1, proxy_size)
            proxy_indices = torch.randperm(len(self.server_proxy_dataset))[:proxy_size].tolist()
            proxy_subset = Subset(self.server_proxy_dataset, proxy_indices)
            proxy_loader = DataLoader(proxy_subset, batch_size=BATCH_SIZE, shuffle=True)
            val_size = int(len(self.server_val_dataset) * data_fraction)
            val_size = max(1, val_size)
            val_indices = torch.randperm(len(self.server_val_dataset))[:val_size].tolist()
            val_subset = Subset(self.server_val_dataset, val_indices)
            ...
            for epsilon in CANDIDATE_EPSILONS:
                with Timer(f"Epsilon {epsilon} Evaluation"):
    ...
    def _calculate_best_epsilon(self, epsilon_results):
        ...
        best_epsilon = max(scores, key=lambda x: x[1])[0]
        log_message(f"Calculated best epsilon based on scores: {best_epsilon}")
        return best_epsilon

def federated(client_datasets, server_proxy_dataset, server_val_dataset, test_loader, num_clients, device):
    with Timer("Total Federated Simulation"):
...
if __name__ == "__main__":
    ...
    def create_dataset(df, img_dir, transform):
        return SkinLesionDataset(df.copy().reset_index(drop=True), img_dir, transform)
